...
interface RefereeHistoryProps<T> {
    refreeId: string;
    title: string;
    dataHistory: T[];
    isLoading: boolean;
    ...
    fetchHistory: (payload: { uid: string, status: FilterProgressStatus }) => void; // new prop
}

const RefereeHistory = <T,>({ refreeId, title, dataHistory, isLoading, trackQuery, children, fetchHistory }: RefereeHistoryProps<T>) => {
    ...
    const fetchRefreedataHistory = () => {
        ...
        fetchHistory({ uid: refreeId, status: (status ? status : "all") });
    }
